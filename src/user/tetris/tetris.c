...
bool check_do_piece_gravity(TetrisGame *tg) {
    ...
    uint64_t curr_time_ms = sys_get_ticks_ms();
    uint64_t elapsed_us = (curr_time_ms - tg->last_gravity_tick_ms) * 1000ULL;

    if (elapsed_us <= tg->gravity_tick_rate_usec) {
        return false;
    }

    tg->last_gravity_tick_ms = curr_time_ms;
